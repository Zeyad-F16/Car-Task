...
import com.example.carTask.exception.InvalidRequestException;
import com.example.carTask.exception.ResourceNotFoundException;
...
public class CarServiceImpl implements CarService {
    ...
    private List<CarDto> carLits = new ArrayList<>();
    ...
    @PostConstruct
    public void initList() throws JsonProcessingException {
        carLits.addAll(Arrays.asList(mapper.readValue(cars, CarDto[].class)));
    }
    ...
    public CarDto getMostExpensive() {
        return carLits.stream()
                .max(Comparator.comparing(CarDto::getPrice))
                .orElseThrow(() -> new ResourceNotFoundException("No cars found"));
    }
    ...
    public void addCar(CarDto car) {
        if (car.getModel() == null || car.getModel().getNameEn() == null || car.getModel().getNameEn().isEmpty()) {
            throw new InvalidRequestException("Car model and name are required");
        }
        carLits.add(car);
    }
    ...
    public void deleteCar(long id) {
        boolean removed = carLits.removeIf(car -> car.getModelID() == id);
        if (!removed) {
    ...
    @Override
    public List<CarDto> searchByModelName(String modelName) {
        return carLits.stream()
                .filter(car -> car.getModel().getNameEn().toLowerCase().contains(modelName.toLowerCase()))
                .toList();
    }
}